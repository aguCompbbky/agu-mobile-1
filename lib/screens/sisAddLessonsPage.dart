...
class _sisAddLessonsPageState extends State<sisAddLessonsPage> {
  ...
  Future<void> _loadSisDataFromDB() async {
    try {
      // 1) Lokal veritabanından oku
      final local = SisLessonsLocalService();
      final data = await local.fetchAllLessonData();

      final data2 = await local.fetchAllLessonData(); // filtersiz
      print('keys: ${data2.keys}'); // burada "compData","ieData"... görmelisin

      // 2) Departman adı -> JSON anahtarı (compData, ieData...)
      final key = getLessonsFromDepartmentName(departmantName) as String;
      final lessons = data[key] ?? <sisLessons>[];

      // 3) İsim listesi
      _lessonsNameList =
          lessons.map((e) => e.ders_adi).whereType<String>().toList();
      _lessonsNameList = _lessonsNameList.toSet().toList(); // uniq
      ...
      printColored(
          "SQLite'den veriler yüklendi: $key (${lessons.length})", "32");
    } catch (e) {
      print("Local DB load error: $e");
